...
class SpatialBlender:
    """
    Return a blending mask using the cross attention produced by both
    source during the inversion and target prompt during editing.
    ...
    def get_mask(self, maps, alpha, use_pool, h=None, w=None, x_t=None, step_in_store: int = None):
        ...
        if self.save_path is not None:
            ...
            if mask.shape[0] == 2:
                ...
                save_mask = mask
            tvu.save_image(rearrange(save_mask.float(), 'c p h w -> p c h w'),
                           save_path,
                           normalize=True)
            self.count += 1
        return mask
    ...
    def __call__(self,
        ...
        Args:
            x_t (_type_): [1,4,8,64,64] # (prompt, channel, video_length, res, res)
        ...
        for item in maps:
            ...
            assert r % (target_h * target_w) == 0 or (target_h * target_w) % r == 0, 'error shape'
            ratio = target_h / target_w
            res_h = int((r * ratio) ** 0.5)
            res_w = int(r / res_h)

            rearranged_item = rearrange(
                item,
                'p c h (res_h res_w) w -> p h c res_h res_w w',
                h=heads,
                res_h=res_h,
                res_w=res_w)
            rearranged_maps.append(
                rearranged_item.to(target_device, dtype=target_dtype))
    ...
    def __init__(self,
        ...
        print('source prompt: ', self.alpha_layers[0][..., 0:(len(prompts[0].split(' ')) + 2)])
        print('target prompt: ', self.alpha_layers[1][..., 0:(len(prompts[1].split(' ')) + 2)])
