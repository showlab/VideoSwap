...
from .loss_utils import get_optical_flow_alpha_loss_all, get_optical_flow_loss_all, get_rigidity_loss
    ...
    with torch.no_grad():
        ...
        for ii in range(40, 500, 80):
            ...
            cv2.putText(reconstruction_texture2.numpy(),
                        'abcdefghijlmnopqrstuvwxyz1234567890!@#$%^&*()-+=>', (10, ii),  # noqa
                        cv2.FONT_HERSHEY_SIMPLEX, 1.2, cur_color, 2, cv2.LINE_AA)
            cv2.putText(reconstruction_texture2.numpy(),
                        'ABCDEFGHIJKLMNOPQRSTUVWXYZ?~;:<./\|][{},', (10, ii + 40),  # noqa
                        cv2.FONT_HERSHEY_SIMPLEX, 1.1, cur_color, 2, cv2.LINE_AA)

        for ii in range(40, 500, 80):
            cur_color = colorsys.hsv_to_rgb((ii - 40) / 500, 1.0, 1.0)
            cv2.putText(reconstruction_texture2.numpy(),
                        'abcdefghijlmnopqrstuvwxyz1234567890!@#$%^&*()-+=>', (10, ii + 500),  # noqa
                        cv2.FONT_HERSHEY_SIMPLEX, 1.2, cur_color, 2, cv2.LINE_AA)
            cv2.putText(reconstruction_texture2.numpy(),
...
def get_mapping_area(model_F_mapping, F_alpha, mask_frames, resx, number_of_frames, uv_shift, device, invert_alpha=False, alpha_thresh=-0.5):
...
def evaluate_model(FG_UV_Mapping, BG_UV_Mapping, F_Atlas, F_Alpha,
                   video_frames, mask_frames, optical_flows, optical_flows_mask,
                   resx, resy, number_of_frames, derivative_amount, uv_mapping_scale,
                   save_dir, device, show_atlas_alpha=False):  #
    ...
    miny = 0
    edge_size = 1
    maxx2, minx2, maxy2, miny2, edge_size2 = get_mapping_area(
        BG_UV_Mapping, F_Alpha, mask_frames > -1, larger_dim, number_of_frames, torch.tensor([-0.5, -0.5]), device, invert_alpha=True)
    maxxt, minxt, maxyt, minyt, edge_sizet = get_mapping_area(
        FG_UV_Mapping, F_Alpha, mask_frames > 0.5, larger_dim, number_of_frames, torch.tensor([0.5, 0.5]), device, invert_alpha=False, alpha_thresh=0.95)

    edited_tex1, texture_orig1 = get_high_res_texture(1000, minx, minx + edge_size, miny, miny + edge_size, F_Atlas, device)

    edited_tex2, texture_orig2 = get_high_res_texture(1000, minx2, minx2 + edge_size2, miny2, miny2 + edge_size2, F_Atlas, device)

    _, texture_orig1t = get_high_res_texture(500, minxt, minxt + edge_sizet, minyt, minyt + edge_sizet, F_Atlas, device)

    _, texture_orig2t = get_high_res_texture(500, minx2, minx2 + edge_size2, miny2, miny2 + edge_size2, F_Atlas, device)

    checkerboard_ = np.array(Image.open(str('videoswap/atlas/checkerboard.png'))).astype(np.float64) / 255.
    ...
    with torch.no_grad():
        for f in tqdm(range(number_of_frames)):
            ...
            for i in range(len(relisa)):
                relis = norm_Scoord_func(torch.from_numpy(relisa[i]).unsqueeze(1))
                reljs = norm_Scoord_func(torch.from_numpy(reljsa[i]).unsqueeze(1))
                # Map video indices to uv coordinates using the two mapping networks:
                uv_temp1 = FG_UV_Mapping(torch.cat((reljs, relis, norm_Tcoord_func(f) * torch.ones_like(relis)), dim=1).to(device))
                uv_temp2 = BG_UV_Mapping(torch.cat((reljs, relis, norm_Tcoord_func(f) * torch.ones_like(relis)), dim=1).to(device))
                # Sample RGB values from the atlas:
                rgb_current1 = F_Atlas(uv_temp1 * 0.5 + 0.5)
                rgb_current2 = F_Atlas(uv_temp2 * 0.5 - 0.5)

                rgb_current1 = (rgb_current1 + 1) * 0.5
                rgb_current2 = (rgb_current2 + 1) * 0.5

                alpha = 0.5 * (F_Alpha(torch.cat((reljs, relis, norm_Tcoord_func(f) * torch.ones_like(relis)), dim=1).to(device)) + 1.0)
                alpha = alpha * 0.99
                alpha = alpha + 0.001
                ...
                rigidity_loss1 = get_rigidity_loss(
                    ...
                    FG_UV_Mapping, uv_temp1, uv_mapping_scale,
                    norm_Scoord_func, norm_Tcoord_func, device, return_all=True)

                rigidity_loss2 = get_rigidity_loss(
                    jif_foreground, derivative_amount, larger_dim,
                    BG_UV_Mapping, uv_temp2, uv_mapping_scale,
                    norm_Scoord_func, norm_Tcoord_func, device, return_all=True)
                ...
                if f < (number_of_frames - 1):
                    flow_loss1 = get_optical_flow_loss_all(
                        ...
                        norm_Scoord_func, norm_Tcoord_func, FG_UV_Mapping, optical_flows, optical_flows_mask,
                        uv_mapping_scale, device, alpha=alpha)

                    flow_loss2 = get_optical_flow_loss_all(
                        jif_foreground, uv_temp2, larger_dim,
                        norm_Scoord_func, norm_Tcoord_func, BG_UV_Mapping, optical_flows, optical_flows_mask,
                ...
                flow_alpha_loss = get_optical_flow_alpha_loss_all(
                    F_Alpha, jif_foreground, alpha, norm_Scoord_func, norm_Tcoord_func, optical_flows, optical_flows_mask, device)
                ...
                rgb21, pointsx1, pointsy1, relevant1 = get_colors(
                    1000, minx, minx + edge_size, miny, miny + edge_size, uv_temp1[:, 0] * 0.5 + 0.5, uv_temp1[:, 1] * 0.5 + 0.5, edited_tex1
                )

                # reconstruct background pixels from the edited background
                rgb22, pointsx2, pointsy2, relevant2 = get_colors(
                    1000, minx2, minx2 + edge_size2, miny2, miny2 + edge_size2, uv_temp2[:, 0] * 0.5 - 0.5, uv_temp2[:, 1] * 0.5 - 0.5, edited_tex2
                )

                # pixels reconstruction from the checkerboard texture:
                rgb21_tex, pointsx1_tex, pointsy1_tex, relevant1_tex = get_colors(
                    500, minxt, minxt + edge_sizet, minyt, minyt + edge_sizet, uv_temp1[:, 0] * 0.5 + 0.5, uv_temp1[:, 1] * 0.5 + 0.5, checkerboard
                )

                rgb22_tex, pointsx2_tex, pointsy2_tex, relevant2_tex = get_colors(
                    500, minx2, minx2 + edge_size2, miny2, miny2 + edge_size2, uv_temp2[:, 0] * 0.5 - 0.5, uv_temp2[:, 1] * 0.5 - 0.5, checkerboard2
                )
                ...
                video_frames_reconstruction_edit1[relisa[i][relevant1], reljsa[i][relevant1], :, f] = rgb21 * alpha.cpu().numpy()[relevant1]
                video_frames_reconstruction_edit2[relisa[i][relevant2], reljsa[i][relevant2], :, f] = rgb22
                video_frames_reconstruction_edit[relisa[i][relevant1], reljsa[i][relevant1], :, f] = \
                    video_frames_reconstruction_edit[relisa[i][relevant1], reljsa[i][relevant1], :, f] + rgb21 * alpha.cpu().numpy()[relevant1]
                video_frames_reconstruction_edit[relisa[i][relevant2], reljsa[i][relevant2], :, f] = \
                    video_frames_reconstruction_edit[relisa[i][relevant2], reljsa[i][relevant2], :, f] + rgb22 * (1 - alpha).cpu().numpy()[relevant2]

                video_frames_reconstruction[relisa[i], reljsa[i], :, f] = rgb_current.detach().cpu().numpy()
                ...
                rgb_error_video[relisa[i], reljsa[i], f] = ((video_frames[relisa[i], reljsa[i], :, f] - rgb_current.cpu()).norm(dim=1) ** 2).numpy()
                rgb_residual_video[relisa[i], reljsa[i], :, f] = ((video_frames[relisa[i], reljsa[i], :, f] - rgb_current.cpu())).numpy()
                video_frames_reconstruction_edit1_checkerboard[relisa[i][relevant1_tex], reljsa[i][relevant1_tex], :, f] = \
                    video_frames_reconstruction_edit1_checkerboard[relisa[i][relevant1_tex], reljsa[i][relevant1_tex], :, f] * \
                    (1 - alpha).cpu().numpy()[relevant1_tex] + rgb21_tex * alpha.cpu().numpy()[relevant1_tex]

                video_frames_reconstruction_edit2_checkerboard[relisa[i][relevant2_tex], reljsa[i][relevant2_tex], :, f] = rgb22_tex
    ...
    mpimg.imsave('%s/texture_edit1.png' % (save_dir), (masks1[:, :, np.newaxis] * edited_tex1.numpy() * (255)).astype(np.uint8))
    mpimg.imsave('%s/texture_orig1.png' % (save_dir), (masks1[:, :, np.newaxis] * texture_orig1.numpy() * (255)).astype(np.uint8))

    mpimg.imsave('%s/texture_edit2.png' % (save_dir), (masks2[:, :, np.newaxis] * edited_tex2.numpy() * (255)).astype(np.uint8))
    if show_atlas_alpha:
        mpimg.imsave('%s/texture_orig1_alpha.png' % (save_dir), (np.concatenate((texture_orig1.numpy(), masks1_alpha[:, :, np.newaxis]), axis=2) * (255)).astype(np.uint8))
        mpimg.imsave('%s/texture_orig2_alpha.png' % (save_dir), (np.concatenate((texture_orig2.numpy(), masks2_alpha[:, :, np.newaxis]), axis=2) * (255)).astype(np.uint8))

    mpimg.imsave('%s/texture_orig2.png' % (save_dir), (masks2[:, :, np.newaxis] * texture_orig2.numpy() * (255)).astype(np.uint8))

    writer_t_edited1 = imageio.get_writer('%s/edited_fg.mp4' % (save_dir), fps=10)
    writer_t_edited2 = imageio.get_writer('%s/edited_bg.mp4' % (save_dir), fps=10)
    writer_t_edited1_tex = imageio.get_writer('%s/edited_fg_tex.mp4' % (save_dir), fps=10)
    writer_t_edited2_tex = imageio.get_writer('%s/edited_bg_tex.mp4' % (save_dir), fps=10)

    writer_alpha = imageio.get_writer('%s/alpha.mp4' % (save_dir), fps=10)

    writer_im_rec = imageio.get_writer('%s/reconstruction.mp4' % (save_dir), fps=10)

    writer_residuals = imageio.get_writer('%s/residuals.mp4' % (save_dir), fps=10)

    writer_alpha_vs_gtmask = imageio.get_writer('%s/alpha_vs_gtmask.mp4' % (save_dir), fps=10)

    writer_edit = imageio.get_writer('%s/edit.mp4' % (save_dir), fps=10)

    writer_uv_1 = imageio.get_writer('%s/uv_fg.mp4' % (save_dir), fps=10)
    writer_uv_1_masked = imageio.get_writer('%s/uv_fg_masked.mp4' % (save_dir), fps=10)
    writer_uv_2 = imageio.get_writer('%s/uv_bg.mp4' % (save_dir), fps=10)

    writer_checkerboard_1 = imageio.get_writer('%s/checkerboard_fg.mp4' % (save_dir), fps=10)
    writer_checkerboard_2 = imageio.get_writer('%s/checkerboard_bg.mp4' % (save_dir), fps=10)

    writer_global_info = imageio.get_writer('%s/global_info.mp4' % (save_dir), fps=10)
    ...
    for i in tqdm(range(number_of_frames)):
        ...
        alpha_vs_mask_rcnn_cur = np.transpose(np.stack(
            (mask_frames[:, :, i].numpy(), alpha_reconstruction[:, :, i], np.zeros_like(mask_frames[:, :, i].numpy()))), (1, 2, 0))

        writer_alpha_vs_gtmask.append_data((alpha_vs_mask_rcnn_cur * 255.0).astype(np.uint8))
